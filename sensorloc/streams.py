...
class Streams (object):
    ...
    def write(self, outputdir, format='MSEED', encoding=None, reclen=None, suffix=''):
        ...
        for trace in self.data:
            if 'encoding' in trace.stats.keys() and encoding is None:
                trace_encoding = trace.stats.encoding
            else:
                trace_encoding = encoding
            fname = os.path.join(outputdir,self.getTraceFilename(trace, 'mseed') + suffix)
            trace.write(filename=fname,format=format,encoding=trace_encoding,reclen=reclen)
